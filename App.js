					...
					<section id="two">
						<header className="major">
							<h2>Sort by technology</h2>
							{
								...
								).map( tag => { return <button className="button small" key={tag}>{tag}</button>})
							}
							<h2>Experiences and projects</h2>
							Work
							<ul>
								{this.profile.work_companies.map((elem)=>{
									return <li key={elem.id}>
												<h3>{elem.label}</h3>
												<ul>
												{this.profile.projects.filter(p => {return p.linked_to === elem.id}).map((p)=>{
													return <li key={p.label}>{p.label}</li>
												})
												}
												</ul>
											</li>
